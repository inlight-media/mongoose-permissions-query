...
describe('mongoose-permissions-query:unit:queryModify', () => {
  ...
  it('should include permitted fields', done => {
    ...
    should.equal(error, null)
    should.equal(newQuery._fields, undefined)
    done()
  })
  ...
  it('should throw error when filter has include:[]', done => {
  ...
  it('should do nothing when filter is exclude:[]', done => {
    const query = Model.findOne()
    const permissionsOptions = {
      filter: {
        population: {
  ...
  it('should not show the fields those are hidden by default query', done => {
    const query = Model.findOne()
  ...
  it('should not show the fields those the default query didnt ask for', done => {
    const query = Model.findOne()
      .select('field3 field4')
    const permissionsOptions = {
      filter: {},
  ...
  it('should not show the banned fields even when default query try to force select it', done => {
    const query = Model.findOne()
      .select('field3 field4')
    const permissionsOptions = {
      filter: {},
