...
function permissionsQuery(defaultQuery, permissionOptions, callback) {
  ...
  if (!_.isEmpty(permissionOptions.filter)) {
    ...
    _.each(permissionOptions.filter, (content, module) => {
      if (_.isArray(content.exclude) && _.isEmpty(content.exclude)) {
      } else if (module === myModule.toLowerCase()) {
...
function filterCheck(filter) {
  ...
  _.each(filter, (content) => {
    ...
    if ((content.include && content.exclude) || (_.isEmpty(content.include) && _.isArray(content.include)) ) {
      output = true
    }
  })
  return output
